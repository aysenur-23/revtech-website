          ...
          <nav className="hidden lg:flex items-center space-x-4 xl:space-x-8">
            {navigation.map((item) => (
              <div
                ...
                onMouseLeave={() => {
                  // Don't close here - mega menu component will handle closing
                }}
              >
                <Link
                  href={item.href}
                  className={`flex items-center space-x-1 px-3 py-2 text-sm font-medium transition-colors hover:text-blue-600 ${pathname === item.href ? 'text-blue-600 font-semibold' : 'text-neutral-800'
                    }`}
                  data-header-nav
                  onMouseEnter={() => {
                    if (item.children) {
                  ...
                  onMouseLeave={() => {
                    // No action needed here - handled by parent div
                  }}
                >
                  <span>{item.name}</span>
                ...
                {item.children && item.name !== (loading ? 'Ürünlerimiz' : t('nav.products')) && item.name !== (loading ? 'Hizmetlerimiz' : t('nav.services')) && (
                  <div
                    className={`absolute left-0 top-full border border-neutral-200 bg-white/95 backdrop-blur-md transition-all duration-200 shadow-xl w-48 ${megaMenuOpen && megaMenuType === 'corporate'
                      ? 'opacity-100 visible'
                      : 'opacity-0 invisible'
                      }`}
                    style={{
                  ...
                  return (
                    <div key={item.name}>
                      <div className="flex items-center justify-between">
                        <Link
                          href={item.href}
                          className={`flex-1 block px-4 py-3.5 sm:py-4 text-base sm:text-lg font-medium text-neutral-900 hover:text-blue-600 hover:bg-neutral-100 rounded-lg transition-colors min-h-[48px] flex items-center touch-manipulation ${!hasChildren ? '' : ''
                            }`}
                          onClick={(e) => {
                            if (hasChildren) {
                              e.preventDefault()
                      ...
                      {item.children && (
                        <div className={`ml-4 sm:ml-6 space-y-1 mt-2 overflow-hidden transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'
                          }`}>
                          {item.children.map((child) => {
                            ...
                            return (
                              <div key={child.name} className="py-1">
                                <div className="flex items-center justify-between">
                                  <Link
                                    href={child.href}
                                    className={`flex-1 block px-4 py-2.5 sm:py-3 text-sm sm:text-base text-neutral-700 hover:text-neutral-900 hover:bg-neutral-100 rounded-md transition-colors min-h-[44px] flex items-center touch-manipulation ${!hasGrandChildren ? '' : ''
                                      }`}
                                    onClick={(e) => {
                                      if (hasGrandChildren) {
                                        e.preventDefault()
                                ...
                                {child.children && (
                                  <div className={`ml-4 sm:ml-6 space-y-1 mt-1 overflow-hidden transition-all duration-300 ease-in-out ${isChildExpanded ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'
                                    }`}>
        ...
        {(megaMenuOpen && megaMenuType === 'products') && (
          <MegaMenu
            ...
            onHoverChange={(hovering) => {
              ...
              if (hovering && leaveTimeout) {
                clearTimeout(leaveTimeout)
                setLeaveTimeout(null)
              }
              // Close menu when user leaves mega menu area
              if (!hovering) {
                setMegaMenuOpen(false)
              }
            }}
